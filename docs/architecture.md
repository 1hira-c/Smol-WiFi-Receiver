...
SlimeVR Server keys devices by the tracker's 48-bit address. Its shared hub
accepts Desktop HID, Android HID, and Wi-Fi sources, then applies one global
RF sequence gate. If USB and Wi-Fi hear the same packet, the first copy wins.
A tracker remains connected while at least one registered source remains.

The gateway is identified by its ESP Wi-Fi station MAC, not its current IP.
Its random boot ID separates restarts. The Server rejects duplicate/reordered
...
- Three seconds without Server ACK clears the dynamic Server selection and
