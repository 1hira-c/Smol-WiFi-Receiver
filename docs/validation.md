...
- Server core: multiple reports, CRC rejection, discovery/offer encoding,
  datagram duplicate/reorder/boot handling, tracker registration, source
  timeout, and cross-HID/Wi-Fi RF sequence deduplication.
- Release builds: nRF54L15 receiver, ESP32-C5 gateway, Tracker XIAO nRF54L15,
  Server core/Desktop/Android, format and SPDX/license checks.
...
3. Record `spiTransferErrors`, `spiDecodeErrors`, ESP queue drops, receiver SPI
   ...
   also remain zero while the Server is connected and ACKing.
4. On a stable LAN, capture generated and Server-accepted sequence counts.
   Delivery must be at least 99.9%, duplicate updates zero, and gateway-added
   latency p99 below 5 ms.
5. Receive one real tracker simultaneously through a legacy USB Receiver and
   the Wi-Fi receiver. Only one device may be created. Disconnect each source
   independently; the tracker disconnects only after the final source ends.
6. Exercise 5 GHz non-DFS and DFS APs, invalid credentials, 30-second setup AP
   fallback, configuration reboot, DHCP address changes, Server restart,
...
Discovery is IPv4 subnet-local. Use the fixed Server field when broadcast or
